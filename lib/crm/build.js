...
cli.main = function ( args, opts ) {
    ...
    if (!Array.isArray(entryDirs)) {
        entryDirs = entryDirs.split(/\s*,\s*/);
    }
    var moduleEntryDirs = [];
    entryDirs.forEach(function (dir) {
        moduleEntryDirs.push(path.resolve(proDir, dir));
    });
    ...
    function callback() {
        ...
        var dirs = [];
        entryDirs.forEach(function (dir) {
            dirs.push(path.resolve(proDir, conf.output, dir));
        });
