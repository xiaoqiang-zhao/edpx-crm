...
function getInfo() {
    ...
    var mockPrefix = 'prefix' in meta 
                        ? meta.prefix 
                        : REMOTE_MOCK_PREFIX;

    var validatorPrefix = 'validator' in meta 
                            ? meta.validator
                            : REMOTE_VALIDTOR_PREFIX;

    mockPrefix += meta.code4mock 
                    ? '/' + meta.code4mock 
                    : '';

    validatorPrefix += meta.code4mock 
                    ? '/' + meta.code4mock 
                    : '';
    ...
    return {
        ...
        remote: {
            host: 'host' in meta ? meta.host : REMOTE_HOST,
            port: 'port' in meta ? meta.port : REMOTE_PORT,
            mock: mockPrefix,
...
mockHanlder.remote = function (context, info) {
    var request = context.request;
    var remoteInfo = getInfo().remote;
    
    var remoteHost = info.host || remoteInfo.host;
    var remotePort = info.port || remoteInfo.port;
    ...
    if (info.path) {
        ...
        request.url = remoteInfo.mock + request.url;
    }
    remote(context);
};
